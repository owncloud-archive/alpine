...
def prepublish(config):
  return [{
    ...
    'settings': {
      ...
      'repo': 'registry.drone.owncloud.com/owncloud/alpine',
...
def sleep(config):
  return [{
    ...
    'commands': [
      'retry -- reg digest --username $DOCKER_USER --password $DOCKER_PASSWORD registry.drone.owncloud.com/owncloud/alpine:%s' % config['internal'],
    ],
  }]
...
def cleanup(config):
  return [{
    ...
    'commands': [
      'reg rm --username $DOCKER_USER --password $DOCKER_PASSWORD registry.drone.owncloud.com/owncloud/alpine:%s' % config['internal'],
    ],
    'when': {
      'status': [
        'success',
